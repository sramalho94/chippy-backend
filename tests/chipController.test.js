...
describe('Chip controller tests', () => {
  ...
  test('get chips by user ID', async () => {
    const response = await request(app).get(`/api/chips/user/${testUserId}`)
  ...
  test('get chips by brand', async () => {
    const response = await request(app).get('/api/chips/brand/Test%20Brand')
  ...
  test('get chips by chip name', async () => {
    const response = await request(app).get('/api/chips/chip-name/Test Chip')
  ...
  test('delete chip', async () => {
    ...
    expect(response.statusCode).toBe(200)
    expect(response.body.message).toBe(`Deleted chip with an id of ${chipId}`)
  })
  afterAll(async () => {
    try {
